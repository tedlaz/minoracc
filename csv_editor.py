...
ALIGN = [qtc.Qt.AlignLeft, qtc.Qt.AlignCenter, qtc.Qt.AlignRight]
...
class CsvTableModel(qtc.QAbstractTableModel):
    ...
    def __init__(self, csv_file):
        ...
        with open(self.filename) as fh:
            ...
            self._headers = ('date', 'apo', 'se', 'poso', 'per')
            self._align = (0, 0, 0, 2, 0)
    ...
    def data(self, index, role):
        ...
        if role in (qtc.Qt.DisplayRole, qtc.Qt.EditRole):
            try:
                return self._data[index.row()][index.column()].strip()
            except IndexError:
                print('Error:', index.row(), index.column())
        if role == qtc.Qt.TextAlignmentRole:
            return ALIGN[self._align[index.column()]]
